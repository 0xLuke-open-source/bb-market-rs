...
pub struct SignalResolver {
    pool: Arc<PgPool>,
    price_source: Arc<dyn SignalPriceSource>,
}

impl SignalResolver {
    pub fn new(pool: Arc<PgPool>, price_source: Arc<dyn SignalPriceSource>) -> Self {
        Self { pool, price_source }
    }

    /// 填充 5m 和 15m 未解析的信号样本
    /// 返回本轮解析条数
    pub async fn resolve_pending(&self) -> Result<usize> {
...
/// 启动信号回填后台任务
pub fn spawn_signal_resolver(resolver: SignalResolver) {
