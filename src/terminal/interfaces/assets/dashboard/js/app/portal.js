...
const PORTAL_PAGES = {
  ...
  proof: {
    kicker: 'Proof of Samples',
    title: '30日样本证明',
    lead: '不展示前端临时估算，直接展示最近 30 日由后端独立样本库沉淀、回写并聚合的真实信号表现。',
    highlights: ['后端独立样本库', '5m / 15m 回写', '衰减时间统计', '覆盖币种与高频标的'],
    metrics: () => []
  },
  about: {
...
const PORTAL_PROOF_WINDOW_DAYS = 30;
const PORTAL_PROOF_TOP_LIMIT = 10;
const PORTAL_PROOF_TTL_MS = 120000;
...
function ensureSalesProofState() {
  if (!S.site) S.site = { page: 'home' };
  if (!S.site.proof) {
    S.site.proof = {
      overview: null,
      loading: false,
      error: '',
      loadedAt: 0,
      windowDays: PORTAL_PROOF_WINDOW_DAYS,
      topLimit: PORTAL_PROOF_TOP_LIMIT,
      requestKey: '',
      promise: null
    };
  }
...
function getSalesProofOverview() {
  return ensureSalesProofState().overview || null;
}

function formatProofCount(value) {
  const num = Number(value);
  if (!Number.isFinite(num)) return '--';
  return new Intl.NumberFormat('zh-CN').format(Math.max(0, Math.round(num)));
}

function formatProofPercent(value, digits = 1) {
  const num = Number(value);
  if (!Number.isFinite(num)) return '--';
  return `${num.toFixed(digits)}%`;
}

function formatProofReturn(value, digits = 2) {
  const num = Number(value);
  if (!Number.isFinite(num)) return '--';
  return `${num >= 0 ? '+' : ''}${num.toFixed(digits)}%`;
}

function formatProofScore(value, digits = 1) {
  const num = Number(value);
  if (!Number.isFinite(num)) return '--';
  return `${num.toFixed(digits)} / 100`;
}

function formatProofMinutes(value) {
  const num = Number(value);
  if (!Number.isFinite(num)) return '--';
  return `${num >= 10 ? num.toFixed(0) : num.toFixed(1)} 分钟`;
}

function formatResolvedProofPercent(value, sampleCount, digits = 1) {
  return Number(sampleCount || 0) > 0 ? formatProofPercent(value, digits) : '--';
}

function formatResolvedProofReturn(value, sampleCount, digits = 2) {
  return Number(sampleCount || 0) > 0 ? formatProofReturn(value, digits) : '--';
}

function formatResolvedProofMinutes(value, sampleCount) {
  return Number(sampleCount || 0) > 0 ? formatProofMinutes(value) : '--';
}
...
function proofShareWidth(value, total) {
  const num = Number(value);
  const den = Number(total);
  if (!Number.isFinite(num) || !Number.isFinite(den) || den <= 0) return 18;
  return Math.max(18, Math.min(100, (num / den) * 100));
}

function proofValueWidth(value, base = 18, scale = 1) {
  const num = Math.abs(Number(value));
  if (!Number.isFinite(num)) return base;
  return Math.max(base, Math.min(100, base + num * scale));
}

function proofSignalTypeLabel(signalType) {
  const normalized = String(signalType || '').toLowerCase();
  if (normalized === 'pump') return '拉升信号';
  if (normalized === 'dump') return '回落信号';
  return signalType || '未知类型';
}

function homeProofHeadline() {
  const state = ensureSalesProofState();
  const overview = state.overview;
  if (overview) {
    return `过去 ${overview.window_days || PORTAL_PROOF_WINDOW_DAYS} 日真实样本，直接回答“提醒有没有参考价值”。`;
  }
  if (state.loading) return '正在读取最近 30 日后端样本证明。';
  if (state.error) return '30 日样本证明暂时读取失败，页面会自动重试。';
  return '最近 30 日样本证明将直接展示后端真实结果，不使用前端临时估算。';
}

function homeProofNarrative() {
  const state = ensureSalesProofState();
  const overview = state.overview;
  if (overview) {
    return `基于后端独立样本库，最近 ${overview.window_days || PORTAL_PROOF_WINDOW_DAYS} 日已沉淀 ${formatProofCount(overview.total_samples)} 个信号样本，覆盖 ${formatProofCount(overview.unique_symbols)} 个币种；其中 ${formatProofCount(overview.resolved_15m_samples)} 个样本已完成 15 分钟结果回写，15m 胜率为 ${formatResolvedProofPercent(overview.win_rate_15m, overview.resolved_15m_samples)}，平均收益为 ${formatResolvedProofReturn(overview.avg_return_15m, overview.resolved_15m_samples)}。最近样本更新时间：${formatProofDateTime(overview.latest_sample_at)}。`;
  }
  if (state.loading) {
    return '页面正在从后端样本库读取最近 30 日样本、覆盖币种、5m / 15m 回写结果与平均衰减时间。';
  }
  if (state.error) {
    return '样本证明页只展示后端真实聚合结果；当前读取失败，不会回退到前端估算口径。';
  }
  return '系统已具备后端独立样本库，这里会持续展示最近 30 日真实样本、解析结果与覆盖情况。';
}

function proofMetricCards(overview) {
  if (!overview) {
    const state = ensureSalesProofState();
    const value = state.loading ? '加载中' : '--';
    const note = state.error || '直接读取后端样本库，不使用前端临时估算。';
    return [
      { label: '30日样本数', value, note, barWidth: 34 },
      { label: '覆盖币种', value, note, barWidth: 42 },
      { label: '15m 已解析', value, note, barWidth: 54 },
      { label: '15m 胜率', value, note, barWidth: 62 },
      { label: '15m 平均收益', value, note, barWidth: 70 },
      { label: '平均衰减时间', value, note, barWidth: 78 }
    ];
  }
  ...
  return [
    {
      label: '30日样本数',
      value: formatProofCount(overview.total_samples),
      note: '后端样本库内最近 30 日累计沉淀的真实信号样本。',
      barWidth: 100
    },
    {
      label: '覆盖币种',
      value: formatProofCount(overview.unique_symbols),
      note: '进入样本库并完成持续跟踪的真实标的数量。',
      barWidth: proofShareWidth(overview.unique_symbols, Math.max(portalRuntimeMetrics().totalSymbols, overview.unique_symbols || 0))
    },
    {
      label: '15m 已解析',
      value: formatProofCount(overview.resolved_15m_samples),
      note: '已完成 15 分钟结果回写的样本数量。',
      barWidth: proofShareWidth(overview.resolved_15m_samples, overview.total_samples)
    },
    {
      label: '15m 胜率',
      value: formatResolvedProofPercent(overview.win_rate_15m, overview.resolved_15m_samples),
      note: '仅对已完成 15 分钟解析的样本统计。',
      barWidth: Math.max(18, Math.min(100, Number(overview.win_rate_15m) || 0))
    },
    {
      label: '15m 平均收益',
      value: formatResolvedProofReturn(overview.avg_return_15m, overview.resolved_15m_samples),
      note: '已解析 15 分钟样本的平均收益表现。',
      barWidth: proofValueWidth(overview.avg_return_15m, 28, 24)
    },
    {
      label: '平均衰减时间',
      value: formatResolvedProofMinutes(overview.avg_decay_minutes, overview.resolved_decay_samples),
      note: '信号从触发到失效的平均持续时间。',
      barWidth: Math.max(18, Math.min(100, 100 - Math.min(Number(overview.avg_decay_minutes) || 0, 30) * 2.2))
    }
  ];
}

function proofOverviewMetrics(overview) {
  if (!overview) {
    const state = ensureSalesProofState();
    const value = state.loading ? '加载中' : '--';
    const note = state.error || '读取最近 30 日后端真实样本。';
    return [
      { label: '30日样本', value, note },
      { label: '覆盖币种', value, note },
      { label: '15m 胜率', value, note },
      { label: '平均衰减', value, note }
    ];
  }
  ...
  return [
    {
      label: '30日样本',
      value: formatProofCount(overview.total_samples),
      note: '最近 30 日后端独立样本库累计沉淀的信号数量。'
    },
    {
      label: '覆盖币种',
      value: formatProofCount(overview.unique_symbols),
      note: '进入样本库并被持续跟踪的真实市场标的数量。'
    },
    {
      label: '15m 胜率',
      value: formatResolvedProofPercent(overview.win_rate_15m, overview.resolved_15m_samples),
      note: `已解析 ${formatProofCount(overview.resolved_15m_samples)} 个 15 分钟样本。`
    },
    {
      label: '平均衰减',
      value: formatResolvedProofMinutes(overview.avg_decay_minutes, overview.resolved_decay_samples),
      note: '信号触发后平均维持有效的时间长度。'
    }
  ];
}

function shouldFetchSalesProof(force = false) {
  const state = ensureSalesProofState();
  if (force) return true;
  if (state.loading) return false;
  if (!state.overview) return true;
  return Date.now() - Number(state.loadedAt || 0) >= PORTAL_PROOF_TTL_MS;
}

async function loadSalesProofOverview(force = false) {
  const state = ensureSalesProofState();
  if (!shouldFetchSalesProof(force)) {
    return state.overview;
  }

  const requestKey = `${PORTAL_PROOF_WINDOW_DAYS}:${PORTAL_PROOF_TOP_LIMIT}:${Date.now()}`;
  state.loading = true;
  state.error = '';
  state.windowDays = PORTAL_PROOF_WINDOW_DAYS;
  state.topLimit = PORTAL_PROOF_TOP_LIMIT;
  state.requestKey = requestKey;

  const task = (async () => {
    try {
      const res = await apiFetch(`/api/proof/overview?days=${PORTAL_PROOF_WINDOW_DAYS}&top=${PORTAL_PROOF_TOP_LIMIT}`);
...
function portalRuntimeMetrics() {
  ...
  const totalSymbols = syms.length || S.access?.total_symbols || 128;
  const visibleSymbols = S.access?.visible_symbols || Math.min(syms.length || 24, 10);
  ...
  const updatingSymbols = syms.filter(s => Number(s.update_count || 0) > 0).length;
  const anomalySymbols = syms.filter(s => Number(s.anomaly_count_1m || 0) > 0).length;
  const highVolSymbols = syms.filter(s => Math.abs(Number(s.change_24h_pct || 0)) >= 5).length;
  ...
  return {
    ...
    anomalySymbols,
    highVolSymbols,
    visibleShare: safePct(visibleSymbols, totalSymbols),
    strongShare: safePct(strongSignals, totalSymbols),
    updateShare: safePct(updatingSymbols, totalSymbols),
    anomalyShare: safePct(anomalySymbols, totalSymbols),
    whaleShare: safePct(whales, totalSymbols),
    eventDensity: totalSymbols > 0 ? (feed.length / totalSymbols).toFixed(1) : '0.0',
...
function homeProofMetrics() {
  return proofMetricCards(getSalesProofOverview());
}
    ...
    <aside class="portal-sidebar">
      ...
      <section class="portal-sidecard">
        ...
        <div class="portal-quicklinks">
          ...
          <button type="button" onclick="switchSitePage('proof')">样本证明</button>
  ...
  return `
    <section class="home-architecture home-reveal" data-reveal-delay="0.08">
      <div class="home-section-head">
        ...
        <div class="home-section-title">把行情接入、结构计算、提醒分发与终端承接，组织成面向活跃交易者的数据链路</div>
      </div>
      <div class="home-arch-layout">
        <div class="home-arch-board">
          ...
          <article class="home-arch-node home-reveal is-node-accent" data-reveal-delay="0.14">
            ...
            <div class="home-arch-node-title">实时行情接入</div>
            <div class="home-arch-node-text">盘口、成交、K 线与异动事件统一接入，形成连续、可追踪的实时数据流。</div>
          </article>
          <article class="home-arch-node home-reveal" data-reveal-delay="0.22">
            <div class="home-arch-node-kicker">计算层</div>
            <div class="home-arch-node-title">结构与因子计算</div>
            <div class="home-arch-node-text">OBI、OFI、主动买卖量差、鲸鱼轨迹与异常结构在这里收敛成统一判断依据。</div>
          </article>
          <article class="home-arch-node home-reveal is-node-primary" data-reveal-delay="0.30">
            <div class="home-arch-node-kicker">决策层</div>
            <div class="home-arch-node-title">提醒决策引擎</div>
            <div class="home-arch-node-text">将复杂市场结构转译为更清晰的强提醒、关注理由与优先级排序。</div>
          </article>
          <article class="home-arch-node home-reveal" data-reveal-delay="0.38">
            <div class="home-arch-node-kicker">分发层</div>
            <div class="home-arch-node-title">提醒分发层</div>
            <div class="home-arch-node-text">实时信号、预警通知与订阅权限在这里完成统一路由与触达。</div>
          </article>
          <article class="home-arch-node home-reveal is-node-accent-alt" data-reveal-delay="0.46">
            ...
            <div class="home-arch-node-title">AI 盯盘终端</div>
            <div class="home-arch-node-text">提醒最终承接到 AI 盯盘终端、分析面板与后续回看流程，形成连续观察闭环。</div>
          </article>
        </div>
        <div class="home-arch-side home-reveal" data-reveal-delay="0.18">
          <div class="home-arch-side-card">
            <div class="home-side-kicker">当前优先关注标的</div>
            <div class="home-side-title">${escapePortalHtml(primary.symbol)}</div>
            <div class="home-side-text">${escapePortalHtml(primary.tag)}，系统会把这类高优先级标的先推进到提醒流与终端焦点位。</div>
          </div>
...
function renderHomeMetricCards(items, valueClass = 'home-stat-value') {
  return items.map((item, index) => `
    <article class="home-stat-card">
      ...
      <div class="home-stat-bar"><span style="width:${Math.max(18,Math.min(100,Number(item.barWidth) || (32 + index * 16)))}%"></span></div>
    </article>
  `).join('');
}
...
function homeBannerSlides(runtime) {
  ...
  const accessLabel = hasFullAccess ? '完整权限已开通' : (isAuthenticated ? '已登录，可继续扩展覆盖' : '公开预览');
  const primaryBannerCta = hasFullAccess
    ? { label: '进入 AI 盯盘', action: "switchSitePage('ai')" }
    : (isAuthenticated
      ? { label: '升级完整权限', action: "switchSitePage('vip')" }
      : { label: '立即体验', action: "openAuthModal('register')" });
  const secondaryBannerCta = hasFullAccess
    ? { label: '查看订阅权益', action: "switchSitePage('vip')" }
    : (isAuthenticated
      ? { label: '进入 AI 盯盘', action: "switchSitePage('ai')" }
      : { label: '登录已有账户', action: "openAuthModal('login')" });
  return [
    {
      key: 'signals',
      kicker: '实时信号体系',
      eyebrow: '更快识别异动',
      title: hasFullAccess ? '让值得关注的异动更早进入视野' : (isAuthenticated ? '已登录，继续扩展完整提醒范围' : '让值得关注的异动更早进入视野'),
      lead: hasFullAccess
        ? '当前账户已具备完整访问权限，可直接进入 AI 盯盘终端承接实时提醒、盘口确认与后续判断。'
        : (isAuthenticated
          ? '当前账户已进入系统访问态，继续升级即可解锁更多标的、完整提醒覆盖与更完整的观察上下文。'
          : '系统把拉升、回落、成交节奏与盘口异动压缩成更清晰的首屏判断，帮助你先看真正值得关注的标的。'),
      ...
      footerTitle: '让提醒成为进入判断的起点，而不是只看一个弹窗',
      footerText: '信号识别、结构确认与终端承接留在同一套界面里，减少在多个页面之间来回切换。',
      bannerCta: '实时信号',
      bars: [58, 82, 96, 72, 114, 94, 126],
      chips: ['信号看板', '盘口结构', '预警引擎'],
      cards: [
        { label: '强提醒候选', value: `${runtime.strongSignals}`, note: '盘中优先级' },
        { label: '事件流', value: `${runtime.feedCount}+`, note: '实时刷新' },
        { label: '覆盖', value: `${runtime.totalSymbols}+`, note: '持续扫描' }
      ]
    },
    {
      key: 'depth',
      kicker: '盘口与成交联动',
      eyebrow: '同屏继续判断',
      title: isAuthenticated ? '把盘口与成交放在同一观察界面' : '提醒之后，继续看盘口和成交确认',
      lead: isAuthenticated
        ? '登录后可直接在同一终端继续查看订单簿、最新成交与结构变化，减少提醒之后的判断断层。'
        : '不是只看到一个提醒，而是继续结合挂单、撤单、吃单与最新成交判断动作真假，避免被单点波动带着走。',
      stateLabel: accessLabel,
      bgStart: '#0B0F16',
      bgEnd: '#1A1610',
      accent: '#20D59E',
      glow: '#F2C760',
      panelTitle: '盘口与成交确认',
      subtitle: `订单簿、最新成交与提醒理由在同一视图里同步承接`,
      linePath: 'M192 502 C256 470 320 442 382 414 C428 394 468 368 514 350 C538 340 552 328 546 316',
      footerTitle: '行情、盘口、成交与提醒在同一终端内连续呈现',
      footerText: '减少从提醒页切到行情页、再切到成交页的成本，让观察动作更连贯。',
      bannerCta: '盘口确认',
      bars: [48, 76, 88, 70, 104, 112, 94],
      chips: ['订单簿', '最新成交', '结构确认'],
      cards: [
        { label: '盘口异常', value: `${runtime.anomalySymbols}`, note: '当前监控' },
        { label: '分析面板', value: '同屏', note: '继续判断' },
    ...
    },
    {
      ...
      kicker: '鲸鱼与资金异动',
      eyebrow: '大额行为追踪',
      title: hasFullAccess ? '把大额行为放回真实市场上下文' : '让鲸鱼异动不再只是单笔大单提示',
      lead: hasFullAccess
        ? '你可以直接把大额挂单、撤单、扫单与盘口变化、最新成交放在同一层观察，过滤噪声动作。'
        : '不是只看一笔大单，而是结合挂单、撤单、吃单与最新成交判断动作真假，减少被单点刺激误导。',
      stateLabel: accessLabel,
      bgStart: '#0A0F16',
      bgEnd: '#141A25',
      accent: '#7CC7FF',
      glow: '#20D59E',
      panelTitle: '鲸鱼异动上下文',
      subtitle: `追踪 ${runtime.whales}+ 类大额异动，并与盘口及成交结构同步交叉验证`,
      linePath: 'M186 510 C236 486 292 456 352 426 C420 390 472 358 546 322',
      footerTitle: '大额挂单、撤单与扫单不再是孤立事件',
      footerText: '把资金轨迹放回盘口与成交上下文里看，更容易判断动作是真承接还是假刺激。',
      bannerCta: '鲸鱼异动',
      bars: [42, 58, 74, 88, 102, 118, 132],
      chips: ['大额挂单', '大额撤单', '主动扫单'],
      cards: [
        { label: '鲸鱼异动', value: `${runtime.whales}`, note: '当前捕捉' },
        { label: '高波动标的', value: `${runtime.highVolSymbols}`, note: '24h 振幅' },
        { label: '结构验证', value: '同层', note: '不孤立看单' }
      ]
    },
    {
      key: 'alerts',
      kicker: '预警通知体系',
      eyebrow: '实时信号 + 告警',
      title: hasFullAccess ? '让预警不止是一个被动通知' : '让预警不再只是被动弹窗',
      lead: hasFullAccess
        ? '完整权限下可直接把预警通知、事件流与实时判断整合到同一工作台里。'
        : '把强提醒、异动通知与事件流放在统一入口，避免提醒碎片化、看完就断链。',
      stateLabel: accessLabel,
      bgStart: '#0C1017',
      bgEnd: '#1C1320',
      accent: '#F97316',
      ...
      cards: [
        ...
        { label: '事件密度', value: `${runtime.eventDensity}x`, note: '每币事件' }
      ]
    },
    {
      key: 'replay',
      kicker: '终端承接与回看',
      eyebrow: '从提醒进入终端',
      title: hasFullAccess ? '从提醒进入终端，继续确认与回看' : '把提醒承接到可继续判断的终端里',
      lead: hasFullAccess
        ? '当前访问层级已足以承接分析面板、终端观察与后续回看，让盘中判断不在提醒之后中断。'
        : '不只给出结论，还把盘口、成交、分析面板与关键时刻回看串成可复用路径。',
      stateLabel: accessLabel,
      bgStart: '#091118',
      bgEnd: '#111B14',
      accent: '#7DD3FC',
      glow: '#20D59E',
      panelTitle: '终端承接闭环',
      subtitle: `把分析面板、订单簿、最新成交与回看入口统一到同一张观察屏`,
      linePath: 'M190 508 C240 488 286 460 344 442 C404 424 458 392 512 356 C532 344 544 332 548 322',
      footerTitle: '从收到提醒到继续判断，不再断链',
      footerText: '关键时刻回看、分析面板与行情上下文留在同一终端语言中协同，观察动作更连续。',
      bannerCta: '终端承接',
      bars: [46, 52, 74, 92, 108, 122, 138],
      chips: ['分析面板', '订单簿', '关键回看'],
      cards: [
        { label: '分析面板', value: '同屏', note: '继续判断' },
        { label: '可见覆盖', value: `${runtime.visibleSymbols}`, note: '当前可见' },
        { label: '工作流', value: '连续', note: '提醒到观察' }
      ]
    }
  ].map(item => ({
    ...item,
...
function refreshHomePortalLive() {
  ...
  const metrics = [
    { label: '实时扫描币种', value: `${runtime.totalSymbols}+`, note: '持续扫描盘口、成交与异动变化' },
    { label: '强提醒候选', value: runtime.strongSignals, note: '当前高优先级关注标的' },
    { label: '鲸鱼异动捕捉', value: runtime.whales, note: '大额进出场轨迹持续刷新' },
    { label: '实时事件流', value: `${runtime.feedCount}+`, note: '提醒、异动与观察入口统一沉淀' }
  ];
  const proofMetrics = homeProofMetrics();
  ...
  setPortalHtml('home-proof-metrics', renderHomeMetricCards(proofMetrics));
  setPortalText('home-proof-title', homeProofHeadline());
  setPortalText('home-proof-text', homeProofNarrative());
...
function renderHomePortalPage() {
  ...
  const trustItems = [
    { label: '更快识别异动', value: '多维联动', note: '把盘口、成交、信号与异动放到同一判断框架。' },
    { label: '更少无效盯盘', value: '条件触发', note: '由系统先扫全市场，再在关键变化出现时提醒你处理。' },
    { label: '实时信号 + 告警', value: '即时提醒', note: '重要变化更早进入视野，而不是事后回看。' },
    { label: '同屏继续判断', value: '终端承接', note: '提醒后直接进入终端，看盘口、成交与当前状态。' }
  ];
  const feedItems = (S.feed || []).slice(0, 8);
  const proofMetrics = homeProofMetrics();
  const capabilities = [
    {
      title: '更快识别值得关注的市场异动',
      body: '不只看价格变化，同时结合盘口、成交与短时结构变化，优先提示更值得关注的标的与时点。'
    },
    {
      title: '把长时间盯盘改成条件触发',
      body: '系统持续扫描，全程代替你做重复观察；当关键变化出现时，再把提醒推到你面前。'
    },
    {
      title: '在同一终端完成观察与判断',
      body: '行情、盘口、成交、信号与预警集中展示，减少在多个页面之间反复切换的成本。'
    },
    {
      title: '实时提醒，不错过关键变化',
      body: '对重要异动进行即时提醒与分级呈现，让你更早进入观察，而不是事后回看才发现。'
    }
  ];
  const stages = [
    { step: '01', title: '让系统先扫全市场', text: '先用综合排序、强提醒和鲸鱼异动筛出真正值得盯的币种。' },
    { step: '02', title: '进入详情确认结构', text: '继续看盘口深度、成交节奏、买卖量差和分析面板，过滤掉假动作。' },
    { step: '03', title: '在关键时刻处理与回看', text: '提醒、观察与后续回看留在同一终端，不需要反复切屏。' }
  ];
  return `
    <section class="portal-page home-page">
      <section class="home-hero home-reveal is-visible">
        ...
        <div class="home-hero-copy">
          <div class="home-hero-kicker">BB-Market / 活跃交易者智能盯盘终端</div>
          <h1 class="home-hero-title">把持续盯盘，变成关键时刻的即时提醒。</h1>
          <p class="home-hero-lead">
            系统持续扫描盘口、成交与市场异动，在值得关注的时点发出提醒，帮助你更快进入观察、更少错过关键变化，
            把更多时间留给判断与执行。
          </p>
          ...
          <div class="home-hero-actions">
            <button class="portal-btn primary" type="button" onclick="${isAuthenticated ? "switchSitePage('ai')" : "openAuthModal('register')"}">${isAuthenticated ? '进入 AI 盯盘' : '立即体验'}</button>
            <button class="portal-btn secondary" type="button" onclick="${isAuthenticated ? "switchSitePage('vip')" : "openAuthModal('login')"}">${isAuthenticated ? (hasFullAccess ? '查看订阅权益' : '升级完整权限') : '登录已有账户'}</button>
          </div>
          <div class="home-hero-manifesto">
            <div class="home-hero-manifesto-head">
              <span class="home-hero-manifesto-kicker">核心价值</span>
              <strong>为活跃交易者提供更快识别异动、更少无效盯盘、更及时收到提醒的统一终端入口。</strong>
            </div>
            <div class="home-hero-manifesto-grid">
              <div class="home-hero-manifesto-item">
                <b>更快识别异动</b>
                <span>把实时信号、鲸鱼轨迹与盘口变化压缩成更清晰的优先级列表，先看值得关注的标的。</span>
              </div>
              <div class="home-hero-manifesto-item">
                <b>更少无效盯盘</b>
                <span>由系统先扫全市场，再在关键变化出现时提醒你处理，把长时间重复观察交给终端。</span>
              </div>
              <div class="home-hero-manifesto-item">
                <b>同屏继续判断</b>
                <span>收到提醒后可直接进入终端，继续看盘口、成交、分析面板与当前市场状态，而不是只看到一个弹窗。</span>
              </div>
            </div>
          </div>
              ...
              isAuthenticated
                ? `
                  <div class="home-hero-form-card access">
                    ...
                    <div class="home-hero-form-title">${hasFullAccess ? '当前账户已开通完整市场访问' : '当前账户已登录，可继续升级完整提醒范围'}</div>
                    <div class="home-hero-form-subtitle">
                      ${hasFullAccess
                        ? `当前权限状态为 ${escapePortalHtml(runtime.accessLabel)}，已开通 ${escapePortalHtml(runtime.totalSymbols)} / ${escapePortalHtml(runtime.totalSymbols)} 个可见标的，可直接进入 AI 盯盘继续观察。`
                        : `当前权限状态为 ${escapePortalHtml(runtime.accessLabel)}，已开通 ${escapePortalHtml(runtime.visibleSymbols)} / ${escapePortalHtml(runtime.totalSymbols)} 个可见标的，可继续扩展完整市场覆盖与提醒范围。`
                      }
                    </div>
                    ...
                    <div class="home-hero-form-actions">
                      ...
                      <button class="portal-btn secondary" type="button" onclick="switchSitePage('vip')">${hasFullAccess ? '查看订阅权益' : '升级完整权限'}</button>
                    </div>
                  </div>
                `
                : `
                  <form class="home-hero-form-card" onsubmit="submitHeroTrial(event)">
                    <div class="home-hero-form-kicker">体验账户开通</div>
                    <div class="home-hero-form-title">快速开通体验账户，先看实时样例，再进入 AI 盯盘终端。</div>
                    <div class="home-hero-form-subtitle">适合活跃交易者先行验证系统是否真的能帮助你更快识别异动、更少无效盯盘，并把提醒承接到同一终端。</div>
                    ...
                    <label class="home-hero-form-field">
                      <span>昵称</span>
                      <input id="hero-trial-display-name" autocomplete="nickname" placeholder="交易者 A">
                    </label>
                    <label class="home-hero-form-field">
                      <span>登录密码</span>
                    ...
                    <div class="home-hero-form-actions">
                      <button class="portal-btn primary" type="submit">立即体验</button>
                      <button class="portal-btn secondary" type="button" onclick="openHeroLogin()">登录已有账户</button>
                    </div>
        ...
        <div class="home-proof-strip">
          <div class="home-proof-head">
            <span class="home-proof-kicker">核心体验</span>
            <strong>用覆盖、刷新与上下文说话，让提醒不止是一个弹窗，而是进入判断的起点。</strong>
          </div>
      ...
      <section class="home-banner home-reveal" data-reveal-delay="0.02">
        <div class="home-section-head compact">
          <div>
            ...
            <div class="home-section-title">围绕活跃交易者真实使用路径组织的五类核心场景</div>
          </div>
      ...
      <section class="home-partners home-reveal" data-reveal-delay="0.04">
        <div class="home-section-head">
          <div class="home-section-kicker">连接生态</div>
          <div class="home-section-title">把行情、提醒、图表与交易工作流接到同一套使用路径</div>
        </div>
      ...
      <section class="home-story home-reveal" data-reveal-delay="0.08">
        <div class="home-story-main">
          <div class="home-section-kicker">最近 30 日后端样本证明</div>
          <div class="home-story-title" id="home-proof-title">${escapePortalHtml(homeProofHeadline())}</div>
          <div class="home-story-text" id="home-proof-text">${escapePortalHtml(homeProofNarrative())}</div>
          <div class="home-story-actions">
            <button class="portal-btn secondary" type="button" onclick="switchSitePage('proof')">查看完整样本证明</button>
          </div>
        </div>
        <div class="home-story-grid proof" id="home-proof-metrics">
          ${renderHomeMetricCards(proofMetrics)}
        </div>
      </section>
      ...
      <section class="home-capabilities home-reveal" data-reveal-delay="0.12">
        <div class="home-section-head">
          <div class="home-section-kicker">核心价值</div>
          <div class="home-section-title">围绕“更快识别、更少盯盘、更及时提醒”展开的四个核心价值</div>
        </div>
      ...
      <section class="home-flow home-reveal" data-reveal-delay="0.16">
        <div class="home-flow-board">
          <div class="home-section-kicker">使用路径</div>
          <div class="home-section-title">从“让系统先看”，到“你在关键时刻处理”的统一使用路径</div>
        ...
        <div class="home-flow-side">
          <div class="home-side-card emphasis">
            ...
            <div class="home-side-title">面向活跃交易者的智能盯盘终端，而不是泛行情信息页。</div>
            <div class="home-side-text">它的重点不是展示更多数据，而是帮助用户更快识别异动、减少无效盯盘，并把提醒承接到可继续判断的终端里。</div>
          </div>
...
function renderProofEmpty(state, fallback) {
  const text = state.loading
    ? '正在从后端样本库读取最近 30 日样本...'
    : (state.error || fallback || '最近 30 日样本仍在累计中。');
  return `<div class="proof-empty">${escapePortalHtml(text)}</div>`;
}

function renderProofSummarySection(overview, state) {
  if (!overview) {
    return `
      <section class="portal-section">
        <div class="portal-section-head">
          <div class="portal-section-title">样本摘要</div>
          <div class="portal-section-desc">这里直接展示后端独立样本库的真实聚合结果。</div>
        </div>
        ${renderProofEmpty(state, '最近 30 日样本证明暂不可用。')}
      </section>
    `;
  }
  ...
  return `
    <section class="portal-section">
      <div class="portal-section-head">
        <div class="portal-section-title">样本摘要</div>
        <div class="portal-section-desc">所有数字都来自后端实时入库后的独立样本库，并按 5m / 15m / 衰减三个时间点持续回写。</div>
      </div>
      <div class="proof-summary-grid">
        <article class="proof-summary-card">
          <span>最近 ${escapePortalHtml(String(overview.window_days || PORTAL_PROOF_WINDOW_DAYS))} 日累计样本</span>
          <b>${escapePortalHtml(formatProofCount(overview.total_samples))}</b>
          <small>每条样本都对应一次真实信号触发事件。</small>
        </article>
        <article class="proof-summary-card">
          <span>已完成 5m / 15m 回写</span>
          <b>${escapePortalHtml(formatProofCount(overview.resolved_5m_samples))} / ${escapePortalHtml(formatProofCount(overview.resolved_15m_samples))}</b>
          <small>结果由后端定时解析，不依赖前端在线状态。</small>
        </article>
        <article class="proof-summary-card">
          <span>最新样本更新时间</span>
          <b>${escapePortalHtml(formatProofDateTime(overview.latest_sample_at))}</b>
          <small>反映最近一次信号写入样本库的时间。</small>
        </article>
      </div>
    </section>
  `;
...
function renderProofSignalTypesSection(overview, state) {
  if (!overview || !Array.isArray(overview.signal_types) || !overview.signal_types.length) {
    return `
      <section class="portal-section">
        <div class="portal-section-head">
          <div class="portal-section-title">信号类型拆解</div>
          <div class="portal-section-desc">按信号方向拆开看样本规模、胜率和平均收益。</div>
        </div>
        ${renderProofEmpty(state, '当前还没有足够的类型样本。')}
      </section>
    `;
  }
  ...
  return `
    <section class="portal-section">
      <div class="portal-section-head">
        <div class="portal-section-title">信号类型拆解</div>
        <div class="portal-section-desc">把拉升与回落样本分开看，避免只看总量却看不清结构差异。</div>
      </div>
      <div class="portal-card-grid proof-type-grid">
        ${overview.signal_types.map(item=>`
          <article class="portal-card proof-type-card">
            <div class="proof-type-head">
              <div>
                <div class="portal-card-title">${escapePortalHtml(proofSignalTypeLabel(item.signal_type))}</div>
                <div class="portal-card-meta">${escapePortalHtml(formatProofCount(item.sample_count))} 个样本</div>
              </div>
              <div class="proof-type-pill">${escapePortalHtml(String(item.signal_type || '').toUpperCase())}</div>
            </div>
            <div class="portal-card-body">最近 30 日内，该类型信号已完成 ${escapePortalHtml(formatProofCount(item.resolved_15m_samples))} 个 15 分钟回写样本，平均触发评分为 ${escapePortalHtml(formatProofScore(item.avg_trigger_score, 1))}。</div>
            <div class="proof-mini-grid">
              <div class="proof-mini-item">
                <span>5m 胜率</span>
                <b>${escapePortalHtml(formatResolvedProofPercent(item.win_rate_5m, item.resolved_5m_samples))}</b>
              </div>
              <div class="proof-mini-item">
                <span>15m 胜率</span>
                <b>${escapePortalHtml(formatResolvedProofPercent(item.win_rate_15m, item.resolved_15m_samples))}</b>
              </div>
              <div class="proof-mini-item">
                <span>5m 平均收益</span>
                <b>${escapePortalHtml(formatResolvedProofReturn(item.avg_return_5m, item.resolved_5m_samples))}</b>
              </div>
              <div class="proof-mini-item">
                <span>15m 平均收益</span>
                <b>${escapePortalHtml(formatResolvedProofReturn(item.avg_return_15m, item.resolved_15m_samples))}</b>
              </div>
            </div>
          </article>
        `).join('')}
...
function renderProofTopSymbolsSection(overview, state) {
  if (!overview || !Array.isArray(overview.top_symbols) || !overview.top_symbols.length) {
    return `
      <section class="portal-section">
        <div class="portal-section-head">
          <div class="portal-section-title">高频样本币种</div>
          <div class="portal-section-desc">样本最密集的标的，适合用来说明系统在高关注市场中的持续覆盖能力。</div>
        </div>
        ${renderProofEmpty(state, '当前还没有可展示的高频样本币种。')}
      </section>
    `;
  }
  ...
  return `
    <section class="portal-section">
      <div class="portal-section-head">
        <div class="portal-section-title">高频样本币种</div>
        <div class="portal-section-desc">按最近 30 日样本数量排序，展示系统最常触发并持续回写的标的。</div>
      </div>
      <div class="portal-table-wrap">
        <table class="portal-table">
          <thead>
          ...
          <tbody>
            ${overview.top_symbols.map(item=>`
              <tr>
                <td>${escapePortalHtml(item.symbol)}</td>
                <td>${escapePortalHtml(formatProofCount(item.sample_count))}</td>
                <td>${escapePortalHtml(formatResolvedProofPercent(item.win_rate_5m, item.resolved_5m_samples))}</td>
                <td>${escapePortalHtml(formatResolvedProofPercent(item.win_rate_15m, item.resolved_15m_samples))}</td>
                <td>${escapePortalHtml(formatResolvedProofReturn(item.avg_return_15m, item.resolved_15m_samples))}</td>
                <td>${escapePortalHtml(formatResolvedProofMinutes(item.avg_decay_minutes, item.resolved_decay_samples))}</td>
                <td>${escapePortalHtml(formatProofDateTime(item.latest_triggered_at))}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
...
function renderProofSidebar(overview, state, runtime) {
  return `
    <aside class="portal-sidebar">
      <section class="portal-sidecard">
        <div class="portal-sidecard-title">统计口径</div>
        <div class="portal-sidecard-kv"><span>窗口范围</span><b>最近 ${escapePortalHtml(String(PORTAL_PROOF_WINDOW_DAYS))} 日</b></div>
        <div class="portal-sidecard-kv"><span>样本状态</span><b>${escapePortalHtml(state.loading ? '读取中' : (state.error ? '读取异常' : '已同步'))}</b></div>
        <div class="portal-sidecard-kv"><span>最新样本</span><b>${escapePortalHtml(formatProofDateTime(overview?.latest_sample_at))}</b></div>
        <div class="portal-sidecard-kv"><span>覆盖币种</span><b>${escapePortalHtml(formatProofCount(overview?.unique_symbols))}</b></div>
        <div class="portal-sidecard-kv"><span>总样本数</span><b>${escapePortalHtml(formatProofCount(overview?.total_samples))}</b></div>
      </section>
      <section class="portal-sidecard">
        <div class="portal-sidecard-title">为什么这页重要</div>
        <div class="proof-side-notes">
          <div class="proof-side-note">
            <b>不是口号</b>
            <span>胜率、收益、衰减时间都来自后端真实写回。</span>
          </div>
          <div class="proof-side-note">
            <b>不是截图</b>
            <span>样本库持续累积，可随着系统运行自动更新。</span>
          </div>
          <div class="proof-side-note">
            <b>不是孤立信号</b>
            <span>样本与 AI 盯盘终端共用同一套实时识别链路。</span>
          </div>
        </div>
      </section>
      <section class="portal-sidecard">
        <div class="portal-sidecard-title">下一步</div>
        <div class="portal-quicklinks">
          <button type="button" onclick="switchSitePage('ai')">进入 AI 盯盘</button>
          <button type="button" onclick="${S.auth?.user ? (S.access?.full_access ? "switchSitePage('vip')" : "subscribeNow()") : "openAuthModal('register')"}">${S.auth?.user ? (S.access?.full_access ? '查看订阅权益' : '升级完整权限') : '注册体验'}</button>
          <button type="button" onclick="switchSitePage('home')">返回首页</button>
          <button type="button" onclick="switchSitePage('about')">了解平台</button>
        </div>
        <div class="proof-side-runtime">当前实时扫描 ${escapePortalHtml(formatProofCount(runtime.totalSymbols))} 个币种，强提醒候选 ${escapePortalHtml(formatProofCount(runtime.strongSignals))} 个。</div>
      </section>
    </aside>
  `;
}

function renderProofPortalPage() {
  const runtime = portalRuntimeMetrics();
  const state = ensureSalesProofState();
  const overview = state.overview;
  const metrics = proofOverviewMetrics(overview);
  const methodology = renderPortalListSection({
    title: '统计口径说明',
    desc: '让访问者清楚这些数字是怎么来的，避免把展示页误解成主观宣传。',
    items: [
      { title: '样本触发口径', text: '每条样本都来自后端实时识别出的真实信号触发事件，触发时立即写入独立样本库。' },
      { title: '表现回写口径', text: '后端持续跟踪样本，并在 5 分钟、15 分钟以及信号失效时分别回写结果。' },
      { title: '页面展示口径', text: '销售页只读取后端聚合结果，不回退到前端临时估算，也不依赖浏览器是否常驻在线。' }
    ]
  });
  const meaning = renderPortalListSection({
    title: '这组数据能回答什么',
    desc: '不是为了展示更多数，而是为了更直接证明系统的实际参考价值。',
    items: [
      { title: '系统是否真的在持续扫描', text: '看样本总量、覆盖币种和高频标的，就能知道系统不是只盯少数几个主流币。' },
      { title: '信号是否具备后续验证链路', text: '看 5m / 15m 已解析样本量与胜率，就能判断这套提醒是否具备后续复核能力。' },
      { title: '提醒是否有持续性而非瞬时噪声', text: '看平均衰减时间与不同类型样本的收益表现，更容易判断系统是否能帮助用户更早进入观察。' }
    ]
  });

  return `
    <section class="portal-page proof-page">
      <div class="portal-hero">
        <div class="portal-kicker">Proof of Samples</div>
        <div class="portal-title">最近 30 日真实样本证明</div>
        <div class="portal-lead">这页只展示后端独立样本库的真实聚合结果。样本由实时信号触发后直接入库，再按 5m / 15m / 衰减三类结果持续回写，适合直接作为销售页里的证据区。</div>
        <div class="portal-highlights">
          <span>真实信号触发入库</span>
          <span>5m / 15m 结果回写</span>
          <span>衰减时间统计</span>
          <span>覆盖币种与高频样本</span>
        </div>
        ${renderPortalActions('proof')}
        <div class="proof-hero-band">
          <div class="proof-hero-band-title">数据状态</div>
          <div class="proof-hero-band-text">${escapePortalHtml(homeProofNarrative())}</div>
          <div class="proof-hero-band-meta">
            <span>样本窗口：最近 ${escapePortalHtml(String(PORTAL_PROOF_WINDOW_DAYS))} 日</span>
            <span>最近样本：${escapePortalHtml(formatProofDateTime(overview?.latest_sample_at))}</span>
            <span>${escapePortalHtml(state.loading ? '状态：读取中' : (state.error ? `状态：${state.error}` : '状态：已同步'))}</span>
          </div>
        </div>
      </div>
      ${renderPortalMetrics(metrics)}
      <div class="portal-layout">
        <div class="portal-main">
          ${renderProofSummarySection(overview, state)}
          ${renderProofSignalTypesSection(overview, state)}
          ${renderProofTopSymbolsSection(overview, state)}
          ${methodology}
          ${meaning}
        </div>
        ${renderProofSidebar(overview, state, runtime)}
      </div>
      ${renderPortalFooter('proof')}
    </section>
  `;
}
...
function mountSitePage(page) {
  ...
  if (page === 'home' || page === 'proof') {
    loadSalesProofOverview();
  }
  if (page === 'home') {
    refreshHomePortalLive();
    startHomePortalMotion();
