...
impl SymbolPanelQueryService {
    ...
    pub async fn load_sales_proof_overview(
        ...
        let client = self.pool.acquire().await?;
        let safe_window_days = window_days.clamp(1, 365);
        let safe_top_limit = i64::try_from(top_symbols_limit.clamp(1, 20)).unwrap_or(8);
        ...
        let overview_row = client
            ...
            .query_one(
                "with base as (
                    select *
                      from market.signal_performance_sample
                     where triggered_at >= now() - ($1::bigint * interval '1 day')
                )
                select
                    count(*)::bigint as total_samples,
                    count(distinct symbol)::bigint as unique_symbols,
                    count(*) filter (where resolved_5m)::bigint as resolved_5m_samples,
                    count(*) filter (where resolved_15m)::bigint as resolved_15m_samples,
                    count(*) filter (where resolved_decay)::bigint as resolved_decay_samples,
                    count(*) filter (where signal_type = 'pump')::bigint as pump_samples,
                    count(*) filter (where signal_type = 'dump')::bigint as dump_samples,
                    coalesce(avg(case
                    ...
                    end), 0.0::double precision) as win_rate_5m,
                    coalesce(avg(case
                    ...
                    end), 0.0::double precision) as win_rate_15m,
                    coalesce(avg(outcome_5m_return) filter (
                        where resolved_5m and outcome_5m_return is not null
                    ), 0.0::double precision) as avg_return_5m,
                    coalesce(avg(outcome_15m_return) filter (
                        where resolved_15m and outcome_15m_return is not null
                    ), 0.0::double precision) as avg_return_15m,
                    coalesce(avg(decay_minutes) filter (
                        where resolved_decay and decay_minutes is not null
                    ), 0.0::double precision) as avg_decay_minutes,
                    coalesce(avg(trigger_score::double precision), 0.0::double precision)
                        as avg_trigger_score,
                    max(triggered_at) as latest_sample_at
                  from base",
                &[&safe_window_days],
            )
            .await?;

        let top_symbol_rows = client
            .client()
            .query(
                "with base as (
                    select *
                ...
                select
                  ...
                  from base
                 group by symbol
                 order by sample_count desc, latest_triggered_at desc, symbol asc
                 limit $2",
                &[&safe_window_days, &safe_top_limit],
            )
            .await?;

        let signal_type_rows = client
            .client()
            .query(
                "with base as (
                    select *
                ...
                select
                 ...
                 order by sample_count desc, signal_type asc",
                &[&safe_window_days],
            )
            .await?;

        let top_symbols = top_symbol_rows
            .into_iter()
            .map(|row| -> std::result::Result<_, tokio_postgres::Error> {
                Ok(SalesProofSymbolRecord {
                    symbol: row.get("symbol"),
                    sample_count: row.get("sample_count"),
                    resolved_5m_samples: row.get("resolved_5m_samples"),
                    resolved_15m_samples: row.get("resolved_15m_samples"),
                    resolved_decay_samples: row.get("resolved_decay_samples"),
                    win_rate_5m: row.try_get("win_rate_5m")?,
                    win_rate_15m: row.try_get("win_rate_15m")?,
                    avg_return_5m: row.try_get("avg_return_5m")?,
                    avg_return_15m: row.try_get("avg_return_15m")?,
                    avg_decay_minutes: row.try_get("avg_decay_minutes")?,
                    avg_trigger_score: row.try_get("avg_trigger_score")?,
                    latest_triggered_at: row.get("latest_triggered_at"),
                })
            })
            .collect::<std::result::Result<Vec<_>, _>>()?;

        let signal_types = signal_type_rows
            .into_iter()
            .map(|row| -> std::result::Result<_, tokio_postgres::Error> {
                Ok(SalesProofSignalTypeRecord {
                    signal_type: row.get("signal_type"),
                    sample_count: row.get("sample_count"),
                    resolved_5m_samples: row.get("resolved_5m_samples"),
                    resolved_15m_samples: row.get("resolved_15m_samples"),
        ...
        Ok(SalesProofOverviewRecord {
            window_days: safe_window_days,
            generated_at: Utc::now(),
            total_samples: overview_row.get("total_samples"),
            unique_symbols: overview_row.get("unique_symbols"),
            resolved_5m_samples: overview_row.get("resolved_5m_samples"),
            resolved_15m_samples: overview_row.get("resolved_15m_samples"),
            resolved_decay_samples: overview_row.get("resolved_decay_samples"),
            pump_samples: overview_row.get("pump_samples"),
            dump_samples: overview_row.get("dump_samples"),
            win_rate_5m: overview_row.try_get("win_rate_5m")?,
            win_rate_15m: overview_row.try_get("win_rate_15m")?,
            avg_return_5m: overview_row.try_get("avg_return_5m")?,
            avg_return_15m: overview_row.try_get("avg_return_15m")?,
            avg_decay_minutes: overview_row.try_get("avg_decay_minutes")?,
            avg_trigger_score: overview_row.try_get("avg_trigger_score")?,
            latest_sample_at: overview_row.get("latest_sample_at"),
            top_symbols,
            signal_types,
        })
    }
...
#[async_trait]
impl SymbolPanelSnapshotStore for SymbolPanelPersistenceService {
    async fn decorate_live_snapshot(
        &self,
        snapshot: &mut SymbolJson,
        signal_history: Vec<FeedEntry>,
    ) {
        SymbolPanelPersistenceService::decorate_live_snapshot(self, snapshot, signal_history).await;
    }

    fn submit_snapshot(&self, snapshot: &SymbolJson, signal_history: Vec<FeedEntry>) {
        SymbolPanelPersistenceService::submit_snapshot(self, snapshot, signal_history);
    }
}

#[async_trait]
impl SymbolPanelSnapshotReader for SymbolPanelQueryService {
    async fn load_recent_snapshots(
        ...
        SymbolPanelQueryService::load_recent_snapshots(self, symbol, limit, from, to).await
    }
    ...
    async fn load_signal_perf_samples(
        ...
        SymbolPanelQueryService::load_signal_perf_samples(self, symbol, limit).await
    }
    ...
    async fn load_sales_proof_overview(
        ...
        SymbolPanelQueryService::load_sales_proof_overview(self, window_days, top_symbols_limit)
            .await
    }
}
...
impl PostgresSymbolPanelSnapshotRepository {
    ...
    async fn ensure_schema(&self) -> Result<()> {
        ...
        client
            ...
            .batch_execute(include_str!(
                "../../../../sql/postgres/market_symbol_panel.sql"
            ))
            .await?;
        client
            .client()
    ...
    async fn insert_signal_factor_detail(
        &self,
        client: &crate::shared::postgres::PooledClient,
        sample_id: Uuid,
        snapshot: &SymbolPanelSnapshotRecord,
        signal_type: &str,
    ) -> Result<()> {
...
fn build_factor_metrics(snapshot: &SymbolJson) -> Vec<FactorMetric> {
    ...
    let strategy = &snapshot.strategy_profile;
    let metrics = vec![
        FactorMetric {
            name: "当前阶段".into(),
            value: format!("{} / {}", strategy.phase, strategy.direction),
            score: strategy.confidence as f64,
            tip: if strategy.reasons.is_empty() {
                strategy.invalidation.clone()
            } else {
                strategy.reasons.join("，")
            },
            tone: if strategy.direction == "偏多" {
                "fg"
            } else if strategy.direction == "偏空" {
                "fr"
            } else {
                "fn"
            }
        ...
        FactorMetric {
            name: "拉盘预期".into(),
            value: format!("{}%", strategy.pump_probability),
            score: strategy.pump_probability as f64,
            tip: format!(
                "后端多窗口策略判断，预计窗口 {} 秒。",
                strategy.expected_window_secs
            ),
            tone: if strategy.pump_probability >= 68 {
                "fg"
            } else if strategy.pump_probability >= 55 {
                "fy"
            } else {
                "fn"
            }
        ...
        FactorMetric {
            name: "砸盘预期".into(),
            value: format!("{}%", strategy.dump_probability),
            score: strategy.dump_probability as f64,
            tip: strategy.invalidation.clone(),
            tone: if strategy.dump_probability >= 68 {
                "fr"
            } else if strategy.dump_probability >= 55 {
                "fy"
            } else {
                "fn"
            }
        ...
        FactorMetric {
            name: "延续概率".into(),
            value: format!("{}%", strategy.continuation_probability),
            score: strategy.continuation_probability as f64,
            tip: "短中周期是否继续向同一方向推进。".into(),
            tone: if strategy.continuation_probability >= 65 {
                "fg"
            } else if strategy.continuation_probability >= 50 {
                "fy"
            } else {
                "fn"
            }
        ...
        FactorMetric {
            name: "假突破风险".into(),
            value: format!("{}%", strategy.false_breakout_probability),
            score: strategy.false_breakout_probability as f64,
            tip: "突破后是否缺少承接和成交确认。".into(),
            tone: if strategy.false_breakout_probability >= 65 {
                "fr"
            } else if strategy.false_breakout_probability >= 50 {
                "fy"
            } else {
                "fn"
            }
        ...
        FactorMetric {
            name: "反转风险".into(),
            value: format!("{}%", strategy.reversal_risk),
            score: strategy.reversal_risk as f64,
            tip: "原方向是否出现衰减，是否开始转向。".into(),
            tone: if strategy.reversal_risk >= 65 {
                "fr"
            } else if strategy.reversal_risk >= 50 {
                "fy"
            } else {
                "fn"
            }
...
fn build_enterprise_metrics(
    ...
    vec![
        EnterpriseMetricSection {
            title: "策略判断".into(),
            subtitle: "阶段、概率与失效条件".into(),
            items: vec![
                metric_row(
                    "当前阶段",
                    strategy.confidence as f64,
                    format!("{} / {}", strategy.phase, strategy.direction),
                    "高级策略引擎对当前盘口所处阶段的判断。",
                    false,
                ),
                metric_row(
                    "拉盘预期",
                    strategy.pump_probability as f64,
                    fmt_metric_value(strategy.pump_probability as f64, "%"),
                    "基于多窗口盘口、主动成交和自适应阈值计算的偏多预期。",
                    false,
                ),
                metric_row(
                    "砸盘预期",
                    strategy.dump_probability as f64,
                    fmt_metric_value(strategy.dump_probability as f64, "%"),
                    "基于多窗口盘口、主动成交和自适应阈值计算的偏空预期。",
                    false,
                ),
                metric_row(
                    "延续概率",
                    strategy.continuation_probability as f64,
                    fmt_metric_value(strategy.continuation_probability as f64, "%"),
                    "短中周期同向推进时，动作继续展开的概率。",
                    false,
                ),
                metric_row(
                    "假突破风险",
                    strategy.false_breakout_probability as f64,
                    fmt_metric_value(strategy.false_breakout_probability as f64, "%"),
                    "突破后若缺少承接和成交确认，这个分值会上升。",
                    true,
                ),
                metric_row(
                    "反转风险",
                    strategy.reversal_risk as f64,
                    fmt_metric_value(strategy.reversal_risk as f64, "%"),
