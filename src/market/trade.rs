...
use crate::web::state::BigTradeJson;
...
impl RecentTradePersistenceService {
    ...
    pub fn submit_trade(&self, record: RecentTradeRecord) {
        ...
        let mut state = self.state.lock().expect("recent trade state mutex poisoned");
        state.insert(record.symbol.clone(), record.agg_trade_id);
    }

    fn should_persist(&self, record: &RecentTradeRecord) -> bool {
        let state = self.state.lock().expect("recent trade state mutex poisoned");
...
impl RecentTradeQueryService {
    ...
    pub async fn load_recent_trades(&self, symbol: &str, limit: usize) -> Result<Vec<BigTradeJson>> {
