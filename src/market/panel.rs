...
impl SymbolPanelPersistenceService {
    pub async fn new(pool: Arc<PgPool>) -> Result<Self> {
        ...
        tokio::spawn(async move {
            let mut tick = tokio::time::interval(Duration::from_secs(PANEL_METRICS_LOG_INTERVAL_SECS));
            loop {
                tick.tick().await;
    ...
    pub async fn decorate_live_snapshot(
        ...
        let perf = self.signal_perf_summary(&snapshot.symbol).await.unwrap_or_default();
    ...
    async fn signal_perf_summary(&self, symbol: &str) -> Result<SignalPerfSummary> {
        {
            let cache = self.perf_cache.lock().expect("panel perf cache mutex poisoned");
            if let Some(entry) = cache.get(symbol) {
                if entry.loaded_at.elapsed()
                    < Duration::from_secs(PANEL_SIGNAL_PERF_CACHE_TTL_SECS)
                {
...
impl PostgresSymbolPanelSnapshotRepository {
    ...
    async fn insert_snapshot(
        ...
        let enterprise_metrics_json =
            apply_signal_perf_to_enterprise_metrics(&snapshot.enterprise_metrics_json, &perf_summary);
    ...
    async fn resolve_signal_samples(&self, snapshot: &SymbolPanelSnapshotRecord) -> Result<()> {
        ...
        for row in rows {
            ...
            if !resolved_decay
                && active_signal_type.as_deref() != Some(signal_type.as_str())
            {
...
fn build_factor_metrics(snapshot: &SymbolJson) -> Vec<FactorMetric> {
    ...
    vec![
        FactorMetric {
            ...
            .into(),
            tone: if ps >= 60.0 { "fg" } else if ps >= 30.0 { "fy" } else { "fn" }.into(),
        },
        FactorMetric {
            name: "下跌压力".into(),
            ...
            .into(),
            tone: if ds >= 60.0 { "fr" } else if ds >= 30.0 { "fy" } else { "fn" }.into(),
        },
        FactorMetric {
            name: "买卖盘失衡".into(),
            ...
            .into(),
            tone: if obi > 10.0 { "fg" } else if obi < -10.0 { "fr" } else { "fn" }.into(),
        },
        FactorMetric {
            name: "主动买入占比".into(),
            value: format!("{:.1}%", tbr),
            ...
            .into(),
            tone: if tbr > 60.0 { "fg" } else if tbr < 40.0 { "fr" } else { "fy" }.into(),
        },
        FactorMetric {
            name: "主动买卖量差".into(),
        ...
        FactorMetric {
            ...
            .into(),
            tone: if ofi > 3_000.0 { "fg" } else if ofi < -3_000.0 { "fr" } else { "fn" }.into(),
        },
        FactorMetric {
            name: "买卖价差".into(),
            ...
            .into(),
            tone: if spread < 10.0 { "fg" } else if spread < 30.0 { "fy" } else { "fn" }.into(),
        },
        FactorMetric {
            name: "大户资金".into(),
            ...
            score: if snapshot.whale_entry { 80.0 } else if snapshot.whale_exit { 60.0 } else { 20.0 },
            ...
            tone: if snapshot.whale_entry { "fg" } else if snapshot.whale_exit { "fr" } else { "fn" }.into(),
        },
        FactorMetric {
            name: "异常波动".into(),
            ...
            .into(),
            tone: if snapshot.anomaly_count_1m > 100 { "fr" } else if snapshot.anomaly_count_1m > 50 { "fy" } else { "fn" }.into(),
        },
    ]
}

fn build_enterprise_metrics(snapshot: &SymbolJson, history: &[MetricPoint]) -> Vec<EnterpriseMetricSection> {
    ...
    let continuity = 100.0 - clamp(avg(history.iter().rev().take(8).map(|item| ((item.tbr - 50.0).abs()) * 2.0)), 0.0, 100.0);
    let directional_continuity = (avg(history.iter().rev().take(8).map(|item| (item.tbr - 50.0) * 2.0))).abs();
    ...
    let count_surge = if prev_big.is_empty() {
        ...
        clamp(recent_big.len() as f64 / prev_big.len() as f64 * 25.0, 0.0, 100.0)
    };
    let amount_surge = if big_prev_notional > 0.0 {
        clamp(big_recent_notional / big_prev_notional * 25.0, 0.0, 100.0)
    } else {
        clamp(big_recent_notional / est_minute_quote.max(1.0) * 100.0, 0.0, 100.0)
    };
    let wall_strength = clamp(snapshot.max_bid_ratio.max(snapshot.max_ask_ratio) * 2.2, 0.0, 100.0);
    let cancel_ratio_est = clamp(snapshot.ofi_raw.abs() / (snapshot.ofi.abs() + 1.0) * 30.0, 0.0, 100.0);
    let recovery_speed = clamp(
        100.0 - (snapshot.spread_bps * 2.0)
            + (metric_delta_pct(last.map(|item| item.bid10).unwrap_or(0.0), prev.map(|item| item.bid10).unwrap_or(0.0)).abs() * 0.3).min(30.0),
        0.0,
        100.0,
    );
    ...
    let multi_tf_consistency = if same_dir.is_empty() {
        ...
        clamp((sum(same_dir.iter().map(|value| value.signum()).collect::<Vec<_>>()).abs() / same_dir.len() as f64) * 100.0, 0.0, 100.0)
    };
    let range_now = cur1m
        .map(|bar| ((bar.h - bar.l) / (if bar.o == 0.0 { 1.0 } else { bar.o })) * 100.0)
        .unwrap_or(0.0);
    let range_avg = avg(bars1m.iter().rev().take(10).map(|bar| ((bar.h - bar.l) / (if bar.o == 0.0 { 1.0 } else { bar.o })) * 100.0));
    let vol_expand = if range_avg > 0.0 {
        clamp(range_now / range_avg * 35.0, 0.0, 100.0)
    } else {
        0.0
    };
    let prev_high = bars1m.iter().rev().take(12).map(|bar| bar.h).fold(0.0, f64::max);
    let prev_low = bars1m.iter().rev().take(12).map(|bar| bar.l).fold(f64::MAX, f64::min);
    let false_break = (current_px > prev_high && cur1m.map(|bar| bar.c).unwrap_or(current_px) < prev_high)
        || (current_px < prev_low && cur1m.map(|bar| bar.c).unwrap_or(current_px) > prev_low);
    let recent_spike_base = bars1m.iter().rev().take(10).last().map(|bar| bar.o).unwrap_or(current_px);
    let recent_extreme = bars1m
        ...
        .fold(0.0, f64::max);
    let pullback = (recent_extreme - ((current_px - recent_spike_base) / recent_spike_base.max(1.0)).abs() * 100.0).abs();
    let acceptance = if current_px >= prev_high * 0.998 || current_px <= prev_low * 1.002 {
        clamp(snapshot.taker_buy_ratio, 0.0, 100.0)
    } else {
        45.0
    };
    let accumulation = if (if snapshot.cvd > 0.0 { 1.0 } else { -1.0 }) * (snapshot.taker_buy_ratio - 50.0) >= 0.0 && snapshot.obi > 0.0 {
        78.0
    ...
    let whale_follow = if snapshot.whale_entry {
        clamp((snapshot.pump_score as f64 + snapshot.taker_buy_ratio) / 1.5, 0.0, 100.0)
    } else if snapshot.whale_exit {
        clamp((snapshot.dump_score as f64 + (100.0 - snapshot.taker_buy_ratio)) / 1.5, 0.0, 100.0)
    } else {
        45.0
    };
    let wall_dwell = if history.len() >= 3 {
        clamp(avg(history.iter().rev().take(6).map(|item| item.wall_bid.max(item.wall_ask))) * 2.0, 0.0, 100.0)
    } else {
        0.0
    };
    ...
    let resonance = clamp((multi_tf_consistency + snapshot.pump_score.max(snapshot.dump_score) as f64) / 2.0, 0.0, 100.0);
    ...
    let liquidity_warning = clamp(snapshot.spread_bps * 1.6 + (100.0 - executable_depth) + snapshot.anomaly_max_severity as f64 * 0.35, 0.0, 100.0);
    let perf = SignalPerfSummary { win5: 0.0, win15: 0.0, count5: 0, count15: 0, decay: 0.0 };
    ...
    vec![
        EnterpriseMetricSection {
            ...
            items: vec![
                metric_row("大单成交占比", clamp(large_trade_ratio, 0.0, 100.0), fmt_metric_value(large_trade_ratio, "%"), "最近1分钟大单成交额，相对日均每分钟成交额的占比。", false),
                metric_row("买卖连续性", clamp(directional_continuity, 0.0, 100.0), fmt_metric_value(directional_continuity, "%"), "主动买卖方向是否持续偏向单边，越高说明延续性越强。", false),
                metric_row("短时成交密度", trade_density, fmt_metric_value(recent_big.len() as f64, "count"), &format!("最近1分钟捕捉到 {} 笔大额成交。", recent_big.len()), false),
                metric_row("笔数突变", count_surge, fmt_metric_value(recent_big.len() as f64 - prev_big.len() as f64, "count"), "对比前1分钟，大额成交笔数的变化幅度。", false),
                metric_row("成交额突变", amount_surge, fmt_metric_value(big_recent_notional, "compact"), "对比前1分钟，大额成交额的变化幅度。", false),
            ],
        },
        EnterpriseMetricSection {
            title: "盘口结构".into(),
            ...
            items: vec![
                metric_row("买卖墙强度", wall_strength, fmt_metric_value(snapshot.max_bid_ratio.max(snapshot.max_ask_ratio), "%"), "前排深度中大额挂单的占比，越高说明墙体越明显。", false),
                metric_row("挂撤单比", cancel_ratio_est, fmt_metric_value(cancel_ratio_est, "%"), "根据深度净变化估算撤单与改单的活跃程度。", true),
                metric_row("恢复速度", recovery_speed, fmt_metric_value(recovery_speed, "%"), "盘口深度被打掉后，重新回补的速度。", false),
                metric_row("前5/10/20档变化", clamp((depth5_delta.abs() + depth10_delta.abs() + depth20_delta.abs()) / 3.0, 0.0, 100.0), format!("{:.0} / {:.0} / {:.0}%", depth5_delta, depth10_delta, depth20_delta), "对比约 20 到 30 秒前，前排深度的变化情况。", false),
                metric_row("深度断层", clamp(depth_gap_bps / 2.0, 0.0, 100.0), fmt_metric_value(depth_gap_bps, "bps"), "相邻档位之间的跳空程度，越高说明深度越不连续。", true),
            ],
        },
        EnterpriseMetricSection {
            title: "价格行为".into(),
            ...
            items: vec![
                metric_row("多周期一致性", multi_tf_consistency, fmt_metric_value(multi_tf_consistency, "%"), "1m、5m、15m、1h 几个周期的方向一致程度。", false),
                metric_row("波动扩张/收缩", vol_expand, fmt_metric_value(vol_expand, "%"), "当前 1m 波动相对最近 10 根 1m 的放大程度。", false),
                metric_row("假突破识别", if false_break { 82.0 } else { 28.0 }, if false_break { "疑似假突破" } else { "暂未发现" }.into(), "破位后快速收回时，追单风险通常会更高。", false_break),
                metric_row("回吐幅度", clamp(pullback * 10.0, 0.0, 100.0), fmt_metric_value(pullback, "%"), "急拉或急砸之后，价格已经回吐的幅度。", true),
                metric_row("新高/低承接", acceptance, fmt_metric_value(acceptance, "%"), "接近新高或新低时，是否仍有主动资金承接。", false),
            ],
        },
        EnterpriseMetricSection {
            title: "资金痕迹".into(),
            ...
            items: vec![
                metric_row("持续吸筹/派发", accumulation, if snapshot.cvd >= 0.0 { "偏吸筹" } else { "偏派发" }.into(), "结合 CVD、买入占比和盘口失衡做出的综合判断。", false),
                metric_row("大户跟随强度", whale_follow, fmt_metric_value(whale_follow, "%"), "大户信号出现后，盘口和主动成交是否继续跟随。", false),
                metric_row("大单停留时间", wall_dwell, fmt_metric_value(wall_dwell, "%"), "大墙挂单在前排停留的时长估算。", false),
                metric_row("主动买卖量差斜率", clamp(cvd_slope.abs(), 0.0, 100.0), fmt_metric_value(cvd_slope, "%"), "主动买卖量差的增长或衰减速度。", false),
            ],
        },
        EnterpriseMetricSection {
            title: "跨周期指标".into(),
            ...
            items: vec![
                metric_row("1m/5m/15m/1h共振", resonance, fmt_metric_value(resonance, "%"), "短中周期信号是否同时偏向同一方向。", false),
                metric_row("短期获中期确认", confirmation, if confirmation >= 60.0 { "已确认" } else { "待确认" }.into(), "短周期异动是否已经获得 5m / 15m 的方向确认。", false),
            ],
        },
        EnterpriseMetricSection {
            title: "交易质量".into(),
            ...
            items: vec![
                metric_row("点差水平", spread_level, fmt_metric_value(snapshot.spread_bps, "bps"), "点差越小，短线执行环境通常越友好。", false),
                metric_row("深度可成交性", executable_depth, fmt_metric_value(executable_depth, "%"), "按 1000 USDT 吃单试算，盘口可立即承接的程度。", false),
                metric_row("滑点风险估计", clamp(slippage_risk * 6.0, 0.0, 100.0), fmt_metric_value(slippage_risk, "bps"), "按 1000 USDT 吃单估算出来的滑点水平。", true),
                metric_row("流动性恶化预警", liquidity_warning, if liquidity_warning >= 70.0 { "偏高" } else { "正常" }.into(), "结合点差、可成交深度和异常波动得出的综合风险。", true),
            ],
        },
        EnterpriseMetricSection {
            title: "信号质量".into(),
            ...
            items: vec![
                metric_row("过去5分钟表现", perf.win5, if perf.count5 > 0 { format!("{:.0}%", perf.win5) } else { "样本少".into() }, "信号触发后 5 分钟内，方向判断的正确率。", false),
                metric_row("过去15分钟表现", perf.win15, if perf.count15 > 0 { format!("{:.0}%", perf.win15) } else { "样本少".into() }, "信号触发后 15 分钟内，方向判断的正确率。", false),
                metric_row("胜率/误报率", perf.win15.max(perf.win5), if perf.count15 > 0 { format!("{:.0} / {:.0}%", perf.win15, 100.0 - perf.win15) } else { "待积累".into() }, "胜率越高说明更稳定，误报率越低说明噪音更少。", false),
                metric_row("信号衰减速度", clamp(100.0 - perf.decay * 6.0, 0.0, 100.0), if perf.decay > 0.0 { format!("{:.1} 分钟", perf.decay) } else { "待积累".into() }, "信号从强提醒回落到普通关注所需的平均时间。", false),
            ],
        },
        EnterpriseMetricSection {
            title: "上下文".into(),
            ...
            items: vec![
                metric_row("连续性稳定度", continuity, fmt_metric_value(continuity, "%"), "最近若干个快照中，主动成交占比的稳定程度。", false),
                metric_row("价格精度", clamp(snapshot.price_precision as f64 * 10.0, 0.0, 100.0), snapshot.price_precision.to_string(), "格式化价格时使用的精度，便于历史回放一致渲染。", false),
                metric_row("数量精度", clamp(snapshot.quantity_precision as f64 * 10.0, 0.0, 100.0), snapshot.quantity_precision.to_string(), "格式化数量时使用的精度，便于历史回放一致渲染。", false),
                metric_row("历史快照样本", clamp(history.len() as f64, 0.0, 100.0), history.len().to_string(), "企业级指标计算已经使用的本地历史样本数。", false),
            ],
        },
    ]
}

fn metric_row(name: &str, score: f64, value: String, tip: &str, invert: bool) -> EnterpriseMetricRow {
    EnterpriseMetricRow {
        name: name.into(),
...
fn depth_gap(levels: &[[f64; 2]]) -> f64 {
    let list = levels.iter().take(8).map(|level| level[0]).filter(|price| *price > 0.0).collect::<Vec<_>>();
    if list.len() < 3 {
        return 0.0;
    }
...
fn get_bars(snapshot: &SymbolJson, interval: &str, count: usize) -> Vec<KlineJson> {
    ...
    bars.into_iter().rev().take(count).collect::<Vec<_>>().into_iter().rev().collect()
}
...
fn calc_return_pct(bars: &[KlineJson]) -> f64 {
    ...
    let first = if bars[0].o != 0.0 { bars[0].o } else { bars[0].c };
    let last = bars.last().map(|bar| bar.c).unwrap_or(0.0);
    if first == 0.0 { 0.0 } else { (last - first) / first * 100.0 }
}
...
fn walk_book_cost(levels: &[[f64; 2]], notional_target: f64) -> WalkBookCost {
    ...
    WalkBookCost { spent, filled_qty, remain }
}
...
fn apply_signal_perf_to_enterprise_sections(
    ...
    if let Some(existing) = sections.iter_mut().find(|section| section.title == "信号质量") {
