...
impl MarketAnalysis {
    pub fn new(book: &OrderBook, features: &OrderBookFeatures) -> Self {
        ...
        // 支撑阻力
        let (support_levels, resistance_levels) = Self::find_support_resistance_advanced(book, features, current_price);

        // 主力意图
        let whale_intent = Self::detect_whale_intent_advanced(features, &advanced_metrics);

        // 预测
        let (short_term, medium_term) = Self::generate_forecasts_advanced(book, features, &advanced_metrics, current_price);

        // 风险提示
        let risk_warnings = Self::generate_risk_warnings_advanced(features, &advanced_metrics);

        // 建议操作
        let recommendations = Self::generate_recommendations_advanced(features, &whale_intent, &short_term, &advanced_metrics);
    ...
    fn calculate_advanced_metrics(book: &OrderBook, features: &OrderBookFeatures, current_price: Decimal) -> AdvancedMetrics {
        // 1. 流动性深度指标
        ...
        let bid_ask_concentration = (features.bid_concentration + features.ask_concentration) / dec!(2);

        // 计算滑点
        let (slippage_buy_1k, slippage_sell_1k) = Self::calculate_slippage(book, current_price, dec!(1000));
        let (slippage_buy_10k, slippage_sell_10k) = Self::calculate_slippage(book, current_price, dec!(10000));

        // 加权价差
        let weighted_spread = features.spread * (dec!(1) + (features.bid_volume_depth + features.ask_volume_depth) / dec!(1000000));
        ...
        let weighted_mid_price = (features.weighted_bid_price + features.weighted_ask_price) / dec!(2);
        ...
        // 恐慌指数等效
        let vix_equivalent = (features.spread_bps * dec!(2) + features.obi.abs() / dec!(2)).min(dec!(100));
        ...
        // 订单簿恢复力
        let order_book_resilience = if features.bid_volume_change.abs() + features.ask_volume_change.abs() > Decimal::ZERO {
            dec!(100) - (features.bid_volume_change.abs() + features.ask_volume_change.abs()) / dec!(2)
        } else {
            dec!(70)
        };
        ...
        let tail_risk_index = (features.max_bid_ratio + features.max_ask_ratio) / dec!(2) * dec!(1.5);
        ...
        let inventory_risk = (features.bid_volume_depth - features.ask_volume_depth).abs() /
            (features.bid_volume_depth + features.ask_volume_depth) * dec!(100);

        let spread_profitability = features.spread_bps * (features.bid_volume_depth + features.ask_volume_depth) / dec!(10000);
    ...
    fn calculate_liquidity_score(book: &OrderBook, features: &OrderBookFeatures) -> Decimal {
        ...
        // 价差评分 (越低分越高)
        if features.spread_bps < dec!(5) { score += dec!(30); }
        else if features.spread_bps < dec!(10) { score += dec!(20); }
        else if features.spread_bps < dec!(20) { score += dec!(10); }
        else if features.spread_bps > dec!(50) { score -= dec!(20); }

        // 深度评分
        let total_depth = features.bid_volume_depth + features.ask_volume_depth;
        if total_depth > dec!(1000000) { score += dec!(30); }
        else if total_depth > dec!(500000) { score += dec!(20); }
        else if total_depth > dec!(100000) { score += dec!(10); }
        else if total_depth < dec!(50000) { score -= dec!(20); }

        // 档位数量评分
        let total_levels = book.bids.len() + book.asks.len();
        if total_levels > 1500 { score += dec!(20); }
        else if total_levels > 1000 { score += dec!(10); }
        else if total_levels < 500 { score -= dec!(10); }

        score.max(dec!(0)).min(dec!(100))
    }

    // 计算滑点
    fn calculate_slippage(book: &OrderBook, start_price: Decimal, amount: Decimal) -> (Decimal, Decimal) {
    ...
    fn calculate_fear_greed_index(features: &OrderBookFeatures) -> Decimal {
        ...
        // OBI贡献
        if features.obi > dec!(30) { score += dec!(20); }
        else if features.obi > dec!(15) { score += dec!(10); }
        else if features.obi < dec!(-30) { score -= dec!(20); }
        else if features.obi < dec!(-15) { score -= dec!(10); }

        // OFI贡献
        if features.ofi > dec!(100000) { score += dec!(15); }
        else if features.ofi < dec!(-100000) { score -= dec!(15); }

        // 波动率贡献
        if features.price_change.abs() > dec!(2) { score += dec!(10); }
        else if features.price_change.abs() < dec!(0.5) { score -= dec!(5); }

        // 鲸鱼活动贡献
        if features.whale_entry { score += dec!(15); }
        if features.whale_exit { score -= dec!(15); }

        score.max(dec!(0)).min(dec!(100))
    }

    // 高级支撑阻力查找
    fn find_support_resistance_advanced(book: &OrderBook, features: &OrderBookFeatures, current_price: Decimal) ->
    (Vec<(Decimal, Decimal, String)>, Vec<(Decimal, Decimal, String)>) {
        ...
        for i in 0..bid_levels.len() {
            ...
            if volume_ratio > dec!(2) {
                ...
                let strength = if volume_ratio > dec!(10) {
                    format!("🔵 强支撑 ({}%, {:.1}%)", volume_ratio.round_dp(1), distance.round_dp(2))
                } else if volume_ratio > dec!(5) {
                    format!("🔹 中支撑 ({}%, {:.1}%)", volume_ratio.round_dp(1), distance.round_dp(2))
                } else {
                    format!("⚪ 弱支撑 ({}%, {:.1}%)", volume_ratio.round_dp(1), distance.round_dp(2))
                };
        ...
        for i in 0..ask_levels.len() {
            ...
            if volume_ratio > dec!(2) {
                ...
                let strength = if volume_ratio > dec!(10) {
                    format!("🔴 强阻力 ({}%, {:.1}%)", volume_ratio.round_dp(1), distance.round_dp(2))
                } else if volume_ratio > dec!(5) {
                    format!("🔸 中阻力 ({}%, {:.1}%)", volume_ratio.round_dp(1), distance.round_dp(2))
                } else {
                    format!("⚪ 弱阻力 ({}%, {:.1}%)", volume_ratio.round_dp(1), distance.round_dp(2))
                };
    ...
    // 高级鲸鱼意图检测
    fn detect_whale_intent_advanced(features: &OrderBookFeatures, advanced: &AdvancedMetrics) -> WhaleIntent {
        // 吸筹特征
        ...
        // 观望特征
        if advanced.aggressive_order_ratio < dec!(10) &&
            advanced.inventory_risk < dec!(20) {
    ...
    // 高级预测生成
    fn generate_forecasts_advanced(book: &OrderBook, features: &OrderBookFeatures,
                                   advanced: &AdvancedMetrics, current_price: Decimal) -> (Forecast, Forecast) {

        let short_term = Self::generate_short_term_forecast_advanced(book, features, advanced, current_price);
        let medium_term = Self::generate_medium_term_forecast_advanced(features, advanced, current_price, &short_term);

        (short_term, medium_term)
    }

    fn generate_short_term_forecast_advanced(book: &OrderBook, features: &OrderBookFeatures,
                                             advanced: &AdvancedMetrics, current_price: Decimal) -> Forecast {
        ...
        // OBI 贡献
        if features.obi > dec!(20) { bullish_score += 4; }
        else if features.obi > dec!(10) { bullish_score += 2; }
        else if features.obi < dec!(-20) { bearish_score += 4; }
        else if features.obi < dec!(-10) { bearish_score += 2; }

        // OFI 贡献
        if features.ofi > dec!(100000) { bullish_score += 5; }
        else if features.ofi > dec!(50000) { bullish_score += 3; }
        else if features.ofi < dec!(-100000) { bearish_score += 5; }
        else if features.ofi < dec!(-50000) { bearish_score += 3; }

        // 订单流质量贡献
        if advanced.order_flow_quality > dec!(70) { bullish_score += 3; }
        else if advanced.order_flow_quality < dec!(30) { bearish_score += 3; }

        // 恐慌指数贡献
        if advanced.fear_greed_index > dec!(70) { bearish_score += 2; } // 过度贪婪
        else if advanced.fear_greed_index < dec!(30) { bullish_score += 2; } // 过度恐惧

        // 价格影响贡献
        if advanced.price_impact_buy > advanced.price_impact_sell { bullish_score += 2; }
        else if advanced.price_impact_sell > advanced.price_impact_buy { bearish_score += 2; }
    ...
    fn generate_medium_term_forecast_advanced(features: &OrderBookFeatures,
                                              advanced: &AdvancedMetrics,
                                              current_price: Decimal,
                                              short_term: &Forecast) -> Forecast {

        let direction = if features.trend_strength > dec!(25) && advanced.fear_greed_index > dec!(60) {
            ForecastDirection::Bullish
        ...
        Forecast {
            ...
            reasoning: format!("趋势强度{:.1}，恐惧贪婪指数{:.1}，流动性评分{:.1}",
    ...
    fn generate_reasoning_advanced(features: &OrderBookFeatures,
                                   advanced: &AdvancedMetrics,
        ...
        match direction {
            ForecastDirection::Bullish | ForecastDirection::StrongBullish => {
                ...
                if advanced.fear_greed_index < dec!(30) {
                    reasons.push(format!("恐惧贪婪指数{:.1} 市场过度恐惧", advanced.fear_greed_index));
                }
            ...
            ForecastDirection::Bearish | ForecastDirection::StrongBearish => {
                ...
                if advanced.fear_greed_index > dec!(70) {
                    reasons.push(format!("恐惧贪婪指数{:.1} 市场过度贪婪", advanced.fear_greed_index));
                }
                if advanced.tail_risk_index > dec!(50) {
                    reasons.push(format!("尾部风险指数{:.1} 风险较高", advanced.tail_risk_index));
                }
            },
            _ => {
    ...
    fn generate_risk_warnings_advanced(features: &OrderBookFeatures, advanced: &AdvancedMetrics) -> Vec<String> {
        ...
        if advanced.liquidity_score < dec!(30) {
            warnings.push(format!("⚠️ 严重流动性不足 - 评分{:.1}", advanced.liquidity_score));
        } else if advanced.liquidity_score < dec!(50) {
            warnings.push(format!("⚠️ 流动性偏低 - 评分{:.1}", advanced.liquidity_score));
        }

        // 滑点风险
        if advanced.quote_slippage_10k > dec!(2) {
            warnings.push(format!("💰 大额交易滑点高 - {:.2}%", advanced.quote_slippage_10k));
        }

        // VaR风险
        if advanced.var_95_1m > dec!(3) {
            warnings.push(format!("📊 1分钟VaR(95%) {:.2}% - 风险较高", advanced.var_95_1m));
        }

        // 尾部风险
        if advanced.tail_risk_index > dec!(60) {
            warnings.push(format!("🐉 尾部风险指数 {:.1} - 警惕极端行情", advanced.tail_risk_index));
        }

        // 库存风险
        if advanced.inventory_risk > dec!(50) {
            warnings.push(format!("📦 库存风险 {:.1}% - 做市商可能调整报价", advanced.inventory_risk));
        }
        ...
        if advanced.price_clustering > dec!(40) {
            warnings.push(format!("🎯 价格聚集度 {:.1}% - 可能存在操纵", advanced.price_clustering));
        }

        // 系统性风险
        if advanced.systemic_risk_index > dec!(50) {
            warnings.push(format!("🌍 系统性风险指数 {:.1} - 关注宏观因素", advanced.systemic_risk_index));
        }
    ...
    // 高级操作建议
    fn generate_recommendations_advanced(features: &OrderBookFeatures,
                                         whale_intent: &WhaleIntent,
                                         forecast: &Forecast,
                                         advanced: &AdvancedMetrics) -> Vec<String> {
        ...
        match forecast.direction {
            ForecastDirection::StrongBullish => {
                ...
                let position_size = if advanced.liquidity_score > dec!(70) { "30-40" } else { "20-30" };
            ...
            ForecastDirection::Bullish => {
                ...
                recs.push(format!("🎯 目标位: {:.6}", forecast.target));
                recs.push(format!("🛑 止损位: {:.6}", forecast.stop_loss));
                let position_size = if advanced.liquidity_score > dec!(70) { "15-20" } else { "10-15" };
                recs.push(format!("📊 仓位建议: {}%", position_size));
            },
            ForecastDirection::Neutral => {
        ...
        if advanced.spread_profitability > dec!(10) {
            recs.push(format!("💰 价差盈利性{:.2}，适合做市", advanced.spread_profitability));
        }
        if advanced.inventory_risk > dec!(40) {
            recs.push("📦 库存风险较高，建议降低持仓".to_string());
        }
        if advanced.optimal_spread > features.spread {
            recs.push(format!("🎯 最优价差{:.6}，可考虑扩大挂单范围", advanced.optimal_spread));
        }

        // 风险管理建议
        if advanced.var_95_1m > dec!(2) {
            recs.push(format!("📊 基于VaR{:.2}%，建议降低杠杆", advanced.var_95_1m));
        }
        if advanced.tail_risk_index > dec!(50) {
