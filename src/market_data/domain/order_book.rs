...
impl HistoryManager {
    ...
    fn update_stats(stats: &mut RollingStats, samples: &VecDeque<RichSamplePoint>) {
        ...
        let variance: Decimal = samples
            ...
            .sum::<Decimal>()
            / n;
        stats.price_std = Decimal::from_f64_retain(variance.to_f64().unwrap_or(0.0).sqrt())
        ...
        if let (Some(cur), Some(prev)) = (samples.back(), samples.front()) {
            ...
            stats.volume_momentum =
                (cur.bid_volume + cur.ask_volume) - (prev.bid_volume + prev.ask_volume);
        }
        if samples.len() >= 3 {
...
impl OrderBook {
    ...
    pub fn apply_incremental_update(&mut self, msg: DepthUpdate) -> anyhow::Result<()> {
        ...
        if let Some((bid, ask)) = self.best_bid_ask() {
            ...
            self.last_total_volume =
                self.bids.values().sum::<Decimal>() + self.asks.values().sum::<Decimal>();
    ...
    pub fn top_n(&self, n: usize) -> (Vec<(Decimal, Decimal)>, Vec<(Decimal, Decimal)>) {
        let top_bids = self
            ...
            .map(|(Reverse(p), q)| (*p, *q))
            .collect();
        let top_asks = self.asks.iter().take(n).map(|(p, q)| (*p, *q)).collect();
        (top_bids, top_asks)
    }

    pub fn get_bids_in_range(&self, start: Decimal, end: Decimal) -> Vec<(Decimal, Decimal)> {
        self.bids
            .range(Reverse(end)..=Reverse(start))
            .map(|(Reverse(p), q)| (*p, *q))
            .collect()
    }
    ...
    pub fn compute_features(&self, depth: usize) -> OrderBookFeatures {
        ...
        let top_bids: Vec<_> = self
            .bids
            .iter()
            .take(depth)
            .map(|(Reverse(p), q)| (*p, *q))
            .collect();
        let top_asks: Vec<_> = self
            .asks
            .iter()
            .take(depth)
            .map(|(p, q)| (*p, *q))
        ...
        let bid_elasticity = if top_bids.len() >= 2 {
            (0..top_bids.len() - 1)
                .map(|i| top_bids[i].0 - top_bids[i + 1].0)
                .sum::<Decimal>()
                / Decimal::from(top_bids.len() - 1)
        } else {
            Decimal::ZERO
        };
        let ask_elasticity = if top_asks.len() >= 2 {
            (0..top_asks.len() - 1)
                .map(|i| top_asks[i + 1].0 - top_asks[i].0)
                .sum::<Decimal>()
                / Decimal::from(top_asks.len() - 1)
        } else {
            Decimal::ZERO
        };
        ...
        // ── 12. 大单占比 ─────────────────────────────────────────
        let max_bid_qty = top_bids
            .iter()
            .map(|(_, q)| *q)
            .fold(Decimal::ZERO, Decimal::max);
        let max_ask_qty = top_asks
            .iter()
            .map(|(_, q)| *q)
            .fold(Decimal::ZERO, Decimal::max);
        ...
        let near_bid_thickness: Decimal = if best_bid > Decimal::ZERO {
            self.get_bids_in_range(best_bid - dec!(0.1), best_bid)
                .iter()
                .map(|(_, q)| q)
                .sum()
        } else {
        ...
        let near_ask_thickness: Decimal = if best_ask > Decimal::ZERO {
            self.get_asks_in_range(best_ask, best_ask + dec!(0.1))
                .iter()
                .map(|(_, q)| q)
                .sum()
        } else {
        ...
        let support_strength: Decimal = if best_bid > Decimal::ZERO {
            self.get_bids_in_range(best_bid * dec!(0.99), best_bid)
                .iter()
                .map(|(_, q)| q)
                .sum()
        } else {
        ...
        let resistance_strength: Decimal = if best_ask > Decimal::ZERO {
            self.get_asks_in_range(best_ask, best_ask * dec!(1.01))
                .iter()
                .map(|(_, q)| q)
                .sum()
        } else {
        ...
        let trend_strength =
            (obi * dec!(0.3) + bid_volume_change * dec!(0.2) + price_change * dec!(0.5))
                .max(dec!(-100))
                .min(dec!(100));
        ...
        let imbalance_acceleration = if self.update_history.len() >= 10 {
            let old = self
                .update_history
                .get(self.update_history.len() - 10)
                .unwrap();
        ...
        // 增量 OFI 买方主导
        else if ofi > dec!(10000) {
            pump_score += 12;
        }
        if obi > dec!(15) {
            pump_score += 20;
        }
        // OBI 偏多（降低门槛，原 pump_signal 没用 OBI）
        else if obi > dec!(8) {
            pump_score += 10;
        }
        if bid_concentration > dec!(25) {
            pump_score += 20;
        } // 买盘集中（原 30 → 25）
        if bid_volume_change > dec!(5) {
            pump_score += 15;
        } // 买单量增加（原 10 → 5）
        if slope_bid > dec!(50000) {
            pump_score += 15;
        } // 买单斜率（原 100000 → 50000）
        if whale_bid && max_bid_ratio > dec!(20) {
            pump_score += 5;
        } // 鲸鱼买入
        ...
        if ask_concentration > dec!(25) {
            dump_score += 20;
        }
        if ask_volume_change > dec!(5) {
            dump_score += 15;
        }
        if slope_ask < dec!(-50000) {
            dump_score += 15;
        }
        if whale_ask && max_ask_ratio > dec!(20) {
            dump_score += 5;
        }
        ...
        let bid_eating = bid_volume_change > dec!(30)
            && ask_volume_change < dec!(-10)
            && price_change > Decimal::ZERO;
        let ask_eating = ask_volume_change > dec!(30)
    ...
    fn calc_liquidity_gap_v(entries: &[(Decimal, Decimal)]) -> usize {
        ...
        for i in 1..entries.len() {
            if (entries[i].0 - entries[i - 1].0).abs() > dec!(0.5) {
                gap += 1;
            }
        }
        gap
