...
use crate::market_data::application::ports::{RecentTradeReader, RecentTradeSink};
...
impl RecentTradeSink for RecentTradePersistenceService {
    fn persist_agg_trade(&self, trade: &AggTrade) {
        self.submit_trade(RecentTradeRecord::from_agg_trade(trade));
    }
}
...
#[async_trait]
impl RecentTradeReader for RecentTradeQueryService {
    async fn load_recent_trades(&self, symbol: &str, limit: usize) -> Result<Vec<BigTradeJson>> {
        RecentTradeQueryService::load_recent_trades(self, symbol, limit).await
    }
}
    ...
    async fn ensure_schema(&self) -> Result<()> {
        ...
        client
            ...
            .batch_execute(include_str!(
                "../../../../sql/postgres/market_recent_trade.sql"
            ))
            .await?;
        Ok(())
    }
