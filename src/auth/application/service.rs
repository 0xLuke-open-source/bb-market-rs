...
use crate::auth::application::dto::{
    AuthRequest, AuthSessionResult, AuthStatusResponse, AuthUserJson, SubscriptionPlanJson,
};
use crate::auth::domain::{AuthRepository, NewSession, NewUserAccount, SubscriptionPlan, UserAccount};
...
impl AuthApplicationService {
    ...
    pub async fn register(&self, req: AuthRequest) -> Result<AuthSessionResult> {
    ...
    pub async fn login(&self, req: AuthRequest) -> Result<AuthSessionResult> {
        let username = normalize_username(&req.username)?;
        let user = self
            .repository
    ...
    pub async fn me_by_token(&self, token: Option<&str>) -> Result<AuthStatusResponse> {
    ...
    pub async fn plans(&self) -> Result<Vec<SubscriptionPlanJson>> {
        let plans = self.repository.list_active_plans().await?;
        Ok(plans.into_iter().map(plan_to_json).collect())
    }
    ...
    pub async fn add_favorite_symbol(&self, token: &str, symbol: &str) -> Result<Vec<String>> {
        ...
        self.repository.add_favorite_symbol(token, &normalized).await
    }

    pub async fn remove_favorite_symbol(&self, token: &str, symbol: &str) -> Result<Vec<String>> {
        let normalized = normalize_symbol(symbol)?;
        self.repository.remove_favorite_symbol(token, &normalized).await
    }
}

fn auth_user(user: &UserAccount) -> AuthUserJson {
    AuthUserJson {
        username: user.username.clone(),
...
fn plan_to_json(plan: SubscriptionPlan) -> SubscriptionPlanJson {
...
fn auth_status(user: Option<AuthUserJson>, stored_user: Option<&UserAccount>) -> AuthStatusResponse {
    let now = Utc::now();
